...
;(function(undefined) {
  ...
  //note: html5 array.reduce(f, inital) 具体可见es5 array.prototype.reduce算法
  if (Array.prototype.reduce === undefined) Array.prototype.reduce = function(fun) {
    if (this === void 0 || this === null) throw new TypeError() // note:Array.prototype.reduce.apply(null) 时抛出异常
    var t = Object(this),
      len = t.length >>> 0,
    //note: undefined >>> 0 == 0
    //note：anything >>> 0的作用
    ...
    //note:和0做或运算也能起到同样的效果,还支持负数
      k = 0,
      accumulator
    if (typeof fun != 'function') throw new TypeError()  //note: 第一个参数必须为函数
    if (len == 0 && arguments.length == 1) throw new TypeError() //note:数组为空 且没有初始值时抛出异常
    //取初始值  
    if (arguments.length >= 2) accumulator = arguments[1] //如果参数长度大于2个，则将第二个参数作为初始值
    else {
        do {
          if (k in t) {  //note:依赖于in的遍历顺序为0,1,2,3，
